...
local function set_filter_formspec(data, meta)
	...
	if data.digiline then
		formspec =
			"size[8.5,3]"..
			"item_image[0.2,0;1,1;liteworks:"..data.name.."]"..
		...
		formspec = "size[8,8.5]"..
			"item_image[0,0;1,1;liteworks:"..data.name.."]"..
...
local function punch_filter(data, filtpos, filtnode, msg)
	...
	local fakePlayer = liteworks.create_fake_player({
		name = owner
	})
	local dir = liteworks.facedir_to_right_dir(filtnode.param2)
	...
	local todir = liteworks.facedir_to_right_dir(filtnode.param2)
	...
	local function grabAndFire(frominvname, filterfor)
		...
		for _, spos in ipairs(sposes) do
			...
			if doRemove > 0 then
				...
				if not todef.tube.can_insert(topos, tonode, item, dir) then return end
				...
				liteworks.inject_item(topos, tonode, item, dir)
...
for _, data in ipairs({
	...
	local node = {
		...
		tiles = {
			"liteworks_"..data.name.."_top.png",
			"liteworks_"..data.name.."_top.png",
			"liteworks_"..data.name.."_output.png",
			"liteworks_"..data.name.."_input.png",
			"liteworks_"..data.name.."_side.png",
			"liteworks_"..data.name.."_top.png",
		},
		paramtype2 = "facedir",
		...
		after_place_node = function (pos, placer)
			...
			--TODO decouple
			--liteworks.after_place(pos)
		end,
		--after_dig_node = liteworks.after_dig,
		--on_rotate = liteworks.on_rotate,
		allow_metadata_inventory_put = function(pos, listname, index, stack, player)
			if not liteworks.may_configure(pos, player) then
				return 0
			end
	...
	if data.digiline then
		...
		node.on_receive_fields = function(pos, formname, fields, sender)
			...
			or not liteworks.may_configure(pos, sender) then
				return
			end
		...
		node.on_receive_fields = function(pos, formname, fields, sender)
			if not liteworks.may_configure(pos, sender) then return end
			fs_helpers.on_receive_fields(pos, fields)
			local meta = minetest.get_meta(pos)
			meta:set_int("slotseq_index", 1)
			set_filter_formspec(data, meta)
	...
	minetest.register_node("liteworks:"..data.name, node)
end

minetest.register_craft( {
	output = "liteworks:filter 2",
	recipe = {
...
minetest.register_craft( {
	output = "liteworks:mese_filter 2",
	recipe = {
	        { "default:steel_ingot", "default:steel_ingot", "basic_materials:plastic_sheet" },
...
if minetest.get_modpath("digilines") then
	minetest.register_craft( {
		output = "liteworks:digiline_filter 2",
		recipe = {
			{ "default:steel_ingot", "default:steel_ingot", "basic_materials:plastic_sheet" },
